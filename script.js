...
const game = {
    ...
    move(event) {
        ...
        /* устанавливаем позицию для змейки и запрашиваем координаты следующей позиции */
        ...
        if (foundFood !== -1) {
            ...
            // добавляем 10 очков к итоговому счету
            score.addTen();
            score.render();
    ...
    /** Функция устанавливает текущий статус игры, раскрашивая контейнер игры в нужный цвет.
     *
...
const snake = {
    ...
    /**
     * Содержит массив объектов с координатами частей тела змейки. По умолчанию: первые три клетки.
    ...
    getNextPosition() {
        ...
        /* если змейка выходит за верхний или нижний край поля, то изменяем координаты
        на противоположную сторону, чтобы змейка выходя за границы возвращалась обратно на поле */
        if (position.top === -1) {
        ...
        /* если змейка выходит за левый или правый край поля, то изменяем координаты
        на противоположную сторону, чтобы змейка выходя за границы возвращалась обратно на поле */
        if (position.left === -1) {
        ...
        // проверка не врезается ли змейка сама в себя
        for (let part of this.parts) {
            if (part.left === position.left && part.top === position.top) {
                game.stop();
            }
        }
...
const score = {
    ...
    onPause() {
        const score = this.getElement();
        score.textContent = 'ПАУЗА';
    },

    onQuit() {
        const score = this.getElement();
        score.style.fontSize = '40px';
        score.textContent = `КОНЕЦ Вы набрали ${this.total} очков`;
    }
};
